...
const GamingCarouselSkeleton = () => <div className="relative w-full py-16 md:py-20" style={{
  ...
  minHeight: "800px"
    ...
    <div className="flex items-center justify-center gap-8 md:gap-10 lg:gap-12 px-5 md:px-10 lg:px-20">
      {[0.85, 0.9, 1, 1.4, 1, 0.9, 0.85].map((scale, i) => <Skeleton key={i} className="rounded-2xl bg-white/30 flex-shrink-0" style={{
...
const GamingCarousel = ({
  ...
  if (parts.length === 0) {
    return <div className="relative w-full overflow-hidden flex flex-col items-center justify-center py-20" style={{
  ...
  return <div className="relative w-full gaming-carousel-container" style={{
    background: "linear-gradient(180deg, #FAFAF8 0%, #F5F3F0 100%)",
    minHeight: "800px"
  }}>
      ...
      scale: 0.95
        ...
        <div className="nav-arrow-glass w-14 h-14 md:w-16 md:h-16 lg:w-20 lg:h-20 flex items-center justify-center rounded-full transition-all duration-300" style={{
      ...
      <motion.button onClick={scrollNext} className="absolute right-4 md:right-8 lg:right-10 top-1/2 -translate-y-1/2 z-20" whileHover={{
      scale: 1.1
    }} whileTap={{
      scale: 0.95
      ...
      <div className="py-8 md:py-10 lg:py-12 px-5 md:px-10 lg:px-20">
        <div className="overflow-visible" ref={emblaRef} style={{
        clipPath: "inset(-100px 0)"
          ...
          <div className="flex gap-8 md:gap-10 lg:gap-12 items-center justify-center">
            ...
            return <div key={part.id} className="flex-shrink-0 transition-all duration-[600ms] ease-out" style={{
              width: getCardWidth(wrappedDistance)
            }}>
                  <GamingCarouselCard part={part} isCenter={wrappedDistance === 0} distanceFromCenter={wrappedDistance} index={index} />
                </div>;
          })}
          </div>
        </div>
      ...
      <div className="flex justify-center gap-2 pb-10">
        {parts.slice(0, Math.min(parts.length, 10)).map((_, index) => <button key={index} onClick={() => emblaApi?.scrollTo(index)} className={`transition-all duration-300 rounded-full ${index === selectedIndex ? "w-8 h-2 bg-mineral shadow-[0_0_12px_rgba(147,181,161,0.6)]" : "w-2 h-2 bg-carbon/20 hover:bg-carbon/40"}`} aria-label={`Aller au produit ${index + 1}`} />)}
        {parts.length > 10 && <span className="text-carbon/40 text-xs ml-2">+{parts.length - 10}</span>}
      </div>
